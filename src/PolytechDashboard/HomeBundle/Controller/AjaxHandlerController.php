...
class AjaxHandlerController extends Controller {
	...
	public function insertTaskAction(Request $request) {
		...
		if ($request->getMethod () == 'POST') {
			...
			/* EntityManager pour faire les requï¿½tes */
			$em = $this->getDoctrine ()->getManager ();
			...
			/* pour valider les persist effectuï¿½es */
			$em->flush ();
			// print_r('after flush');
		}
	...
	public function removeTaskAction(Request $request) {
		...
		$query->execute ();
		$criteria = Criteria::create ()->where ( Criteria::expr ()->eq ( "idtache", $request->get ( 'id' ) ) )->setFirstResult ( 0 )->setMaxResults ( 20 );
		$em->flush ();
	...
	public function updateTaskAction(Request $request) {
		$Tache = $this->getDoctrine ()->getRepository ( 'PolytechDashboardHomeBundle:Tache' )->findOneBy ( array (
				'id' => $request->get ( 'TaskForm_idTask_form8' ) 
		) );
	...
	public function getMytasksRendredAction() {
		...
		/* liste de taches affectï¿½es a l'etudiant */
		...
		if ($tabTasks) {
			...
			for($i = 0; $i < $max; $i ++) {
				...
				if ($admin != null) {
					/* tache affectï¿½e par un gestionnaire */
		...
		if ($tabTasksSend) {
			...
			for($i = 0; $i < $max; $i ++) {
				...
				/* tache affectï¿½e a un gestionnaire */
	...
	public function removeValidTaskAction(Request $request) {
		$em = $this->getDoctrine ()->getEntityManager ();
		$criteria = Criteria::create ()->where ( Criteria::expr ()->eq ( "idetudiant", $this->getRequest ()->getSession ()->get ( 'loginTMP' )->getId () ) )->andWhere ( Criteria::expr ()->eq ( "status", "VALIDE" ) );
		
		$taches = $this->getDoctrine ()->getRepository ( 'PolytechDashboardHomeBundle:Tacheetudiant' )->matching ( $criteria );
	...
	public function removeSendedValidTaskAction(Request $request) {
		...
		$taches = $this->getDoctrine ()->getRepository ( 'PolytechDashboardHomeBundle:Tachegestionnaire' )->matching ( $criteria );
